...
TEST_CASE("TestSimpleHelp")
{
    ...
    try {
        ...
        Arg host(SL('s'), SL("host"), true, true);
        host.setDescription(
            SL("Host. Can be \"localhost\", "
               "\"any\" or regular IP."));
        host.setLongDescription(
            SL("Host. This argument told to the "
        ...
        timeout.setDescription(
            SL("Timeout before new messages will be "
...
{
    const int argc = 2;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h=-s")};
...
{
    const int argc = 2;
    const CHAR *argv[argc] = {SL("program.exe"), SL("--hots")};
...
TEST_CASE("TestHelpWithCommands")
{
    ...
    try {
        ...
        Help help;
        help.setAppDescription(SL("This application just show power of the Args help."));
        help.setExecutable(SL("executable"));
...
{
    const int argc = 2;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h=add")};
...
{
    const int argc = 3;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h=add"), SL("-f")};
...
{
    const int argc = 3;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h=add"), SL("file")};
    ...
    try {
        ...
        file.setLongDescription(
            SL("Add file. File can exist but if "
...
#else
        REQUIRE(g_argsOutStream.str()
                == SL("Add file. File can exist but if it's not so new file will be created. \n"
                      "\n"
                      "USAGE: file <arg> <options>\n"
                      "\n"
                      "GLOBAL OPTIONAL:\n"
...
TEST_CASE("TestHelpOfCommand")
{
    ...
    try {
        ...
        Command add(SL("add"));
        add.setDescription(SL("Add new file."));
        add.setLongDescription(
            SL("Add file. File can exist but if it's "
...
{
    const int argc = 3;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add")};
...
TEST_CASE("TestRequiredAllOfGroupHelp")
{
    ...
    try {
        ...
        cmd.addAllOfGroup(SL("group"), true)
            .addArgWithFlagOnly(SL('a'), true, false, SL("Argument."))
            .end()
...
TEST_CASE("TestFlagWithBigVSHelp")
{
    ...
    try {
        ...
        cmd.addArgWithFlagOnly(SL('a'), true, false, SL("Argument."), SL(""), SL(""), SL("bin|lib|shared"))
            .addHelp(true, SL("executable"), SL("Test help."));
...
TEST_CASE("TestFlagAndNameWithBigVSHelp")
{
    ...
    try {
        ...
        cmd.addArgWithFlagAndName(SL('a'),
                                  SL("arg"),
                                  true,
                                  false,
                                  SL("Argument."),
...
{
    const int argc = 4;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("dummy")};
...
TEST_CASE("TestHelpWithCommands")
{
    ...
    cmd.addCommand(SL("add"), ValueOptions::NoValue, true, SL("Add item to..."))
        .addCommand(SL("file"), ValueOptions::OneValue, false, SL("File item."), SL(""), SL(""), SL("fn"))
        .end()
        .end()
...
{
    const int argc = 4;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("file")};
...
{
    const int argc = 5;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("file"), SL("dummy")};
...
{
    const int argc = 5;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("file"), SL("-w")};

    CmdLine cmd(argc, argv);

    cmd.addCommand(SL("add"), ValueOptions::NoValue, true, SL("Add item to..."))
        .addCommand(SL("file"), ValueOptions::OneValue, false, SL("File item."), SL(""), SL(""), SL("fn"))
        .addArgWithFlagAndName(SL('w'),
                               SL("work"),
                               false,
                               false,
...
{
    const int argc = 5;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("dummy"), SL("dummy")};
...
{
    const int argc = 4;
    const CHAR *argv[argc] = {SL("program.exe"), SL("-h"), SL("add"), SL("-f")};
...
TEST_CASE("TestHelpCommandWithValue")
{
    ...
    cmd.addCommand(SL("add"), ValueOptions::OneValue, false, SL("Add item to..."), SL(""), SL(""), SL("item"))
        .end()
        .addHelp(true, SL("program.exe"), SL("Help with commands."));
...
{
    const int argc = 2;
    const CHAR *argv[argc] = {SL("program"), SL("-h")};

    CmdLine cmd(argc, argv);

    cmd.addCommand(SL("additemtothedatabase"), ValueOptions::NoValue, true, SL("Add item to the database..."))
        .end()
